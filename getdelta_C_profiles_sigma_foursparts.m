function [leftProfiles, rightProfiles] = getdelta_C_profiles_sigma_foursparts
...
%sorts them (delta C<=-1sigma, -1sigma<delta C<=mean, mean<delta C<=+1sigma, stimC>+1sigma) using matrices to place them.
...
lSigma_to_mean_idx = (-1<deltaC_zScore & deltaC_zScore<=0); 
mean_to_rSigma_idx = (0<deltaC_zScore & deltaC_zScore<=1); 
...
legend('delta Criterion 1 Sigma Below Mean','', 'delta Criterion Between -1 Sigma and Mean','', ...
    'delta Criterion Between mean and +1 Sigma','','delta Criterion 1 Sigma Above Mean','Fontsize',5,'Location','southwest');
ax = gca;
xlim(ax, [0, bins]);
ax.XGrid = 'on';
title('\fontsize{11}Mean Profiles of Delta Criterion for +/- 1 Sigma and Inbetween Mean');
