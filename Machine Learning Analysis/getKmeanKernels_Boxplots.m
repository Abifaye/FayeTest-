...
load('masterClusters_noRTs.mat')%changeable
...
for nProfiles = 1:length(profile_names)
    ...
    %plots
    nexttile(nProfiles); %or tileorder(nProfiles) if wanted to be ordered
    hold on
...
Vars = [5 7:9]; %vars in masterDBTable to reiterate through
for nVar = 1:length(Vars)
    nexttile(nVar); %or tileOrder(nVar)
