...
%
clrNum = DBHit(4).(string(Fields(DBdata)));
...
figure;
gscatter(hitDataTable.(xVar),hitDataTable.(yVar),DBHit(1).(string(Fields(DBdata))),clr)%check if using xVar and yVar works
...
while addPlot == 1
    xVar = listdlg('PromptString',{'Select x-var'},'ListString',T.Properties.VariableNames,'SelectionMode','single');
    yVar = listdlg('PromptString',{'Select y-var'},'ListString',T.Properties.VariableNames,'SelectionMode','single');
    
   figure;
gscatter(DBHit.(xVar),DBHit.(yVar),DBHit(1).(string(Fields(DBdata))),clr)%check if using xVar and yVar works
title(string(Fields(DBdata)))
xlabel(DBHit.Properties.VariableNames{xVar})
ylabel(DBHit.Properties.VariableNames{yVar})
    addPlot = input(strcat('Create another plot for current data? [Y=1/N=0]:',32));
end
