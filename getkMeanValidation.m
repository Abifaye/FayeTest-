...
if grouping==0%Run Kmeans, all Data Combined
    for nKMeans = 1:length(compareClusters)%reiterate through all indicated Ks
        ...
        for nCluster = 1:compareClusters(nKMeans) %reiterate through each cluster
            masterD.(nCluster) = pdist2(Clusters(Clusters==nCluster),Centroids(Centroids==nCluster)); %computes distance of each point from centroid for each cluster
        end
        masterInertia.(string(compareClusters(nKMeans))) = sum(table2array(masterD));%calculates inertia for Kmean with certain number of clusters
    end
end
